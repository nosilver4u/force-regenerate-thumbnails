...
<div class="wrap regenthumbs">
	...
	// <![CDATA[
		jQuery(document).ready(function($){
			var i;
			var rt_images = [<?php /* echo esc_html( $ids ); */ ?>];
			...
			function RegenThumbsFinishUp() {
				...
				if (rt_errors > 0) {
					rt_resulttext = '<?php /* echo wp_kses_post( $text_failures ); */ ?>';
				} else {
					rt_resulttext = '<?php /* echo wp_kses_post( $text_nofailures ); */ ?>';
				}
	...
	function ajax_process_image() {
		...
		try {
			 ...
			 * Display results
			 */
			$message  = sprintf( __( '<b>&quot;%s&quot; (ID %d)</b>', 'force-regenerate-thumbnails' ), esc_html( get_the_title( $id ) ), $image->ID );
